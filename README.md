...
템플릿 원본과 `rules.yaml` 은 별도의 저장소에 모아 두지 않는다. 고객사
프로젝트마다 생기는 Project Repository 안에 그 프로젝트의 산출물과 함께
`templates/` 와 `rules/` 로 둔다. 엔진은 검사할 문서에서 위로 올라가며 이
둘을 함께 든 폴더를 찾으므로, 기준을 찾으러 Project Repository 밖으로 나갈
일이 없다.

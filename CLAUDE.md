...
4. **템플릿 원본은 여기 없다.** 고객사별 템플릿과 규칙 파일은 별도의 문서 저장소에서
   관리한다. 저장소 하나 안에 회사별 폴더를 두고, 각 폴더가 `templates/` 와 `rules/` 와
   `docs/` 를 들고 있다. 엔진은 자기 저장소를 검사하지 않고 그 저장소가 엔진을 가져다 쓴다.

### 지금 어디까지 왔나

검사 엔진 MVP 가 동작한다. 규칙 종류 여섯(파일명, 위치, 필수 절, 필수 시트, 표 헤더,
슬라이드 레이아웃)과 리더 넷(md, docx, xlsx, pptx), `check` CLI 가 들어 있다.

거절하는 껍데기도 둘 다 있다. 플러그인 훅이 팀원 PC 에서 저장을 막고, GitHub Actions 가
커밋 시점에 PR 을 실패시킨다. 오피스 문서는 훅이 불리지 않으므로 Actions 쪽이 맡는다.

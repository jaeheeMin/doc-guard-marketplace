...
아래 여섯 문서가 이 저장소의 규칙 전문이다. 이 문서는 요약만 담고, 정확한
...
| `rules/layout.md` | 디렉터리별 용도와 새 파일을 어디에 둘지 판단하는 기준 |
...
| `/release` | 릴리즈를 발행할 때. 이슈와 PR 을 모아 릴리즈 노트를 쓰고 GitHub Release 를 만든다 |
| `/wrapup` | 세션에서 다 끝내지 못한 작업을 이슈로 남길 때 |
| `/intake` | 기존 작업 디렉터리와 요구사항을 받아 이 저장소를 실제 내용으로 채울 때 |
